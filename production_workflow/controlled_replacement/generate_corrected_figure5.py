...
ERROR_ROOT = Path(os.environ.get("JOG_CONTROLLED_MAP_FIELDS", Path(__file__).resolve().parent / "map_fields"))
OUTPUT_ROOT = Path(os.environ.get("JOG_CONTROLLED_FIGURES", Path(__file__).resolve().parent / "figures"))
CONFIGS = ("CFG02", "CFG04")
DISPLAY_GRID_M = 1800.0
DATASET = Path(os.environ.get(
    "JOG_CANONICAL_DATASET",
    ROOT / "production_workflow/gate2_results/gate2_canonical_dataset_20260820_c/canonical_master_dataset.csv.gz",
))
...
def verified_error_fields(config: str) -> pd.DataFrame:
    manifest_path = ERROR_ROOT / "map_field_export_manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("status") != "complete":
        raise RuntimeError("Footprint-error export is not complete")

    source = ERROR_ROOT / f"{config}_ten_square_controlled_fields.npz"
    ...
    with np.load(source, allow_pickle=False) as data:
        ...
        uniform_error = data["uniform_error"].astype(float)
        frame = pd.DataFrame({
...
def aligned_inversion_error(frame: pd.DataFrame) -> pd.DataFrame:
    """The corrected export already contains the aligned inversion residual."""
    if not np.isfinite(frame["inversion_error"]).all():
        raise RuntimeError("Non-finite corrected inversion residual")
    return frame
...
def main() -> None:
    ...
    output_manifest = {
        "schema": "jog-controlled-replacement-figure5-v1",
        "status": "complete",
        ...
        "source_manifest": "map_field_export_manifest.json",
        "source_hashes": {
            f"{cfg}_ten_square_controlled_fields.npz": sha256(
                ERROR_ROOT / f"{cfg}_ten_square_controlled_fields.npz"
            )
            for cfg in CONFIGS
        },
    }
