#!/usr/bin/env python3
"""Run all controlled-replacement jobs with one reused Icepack context.
...
def main() -> None:
    ...
    if registry.get("schema") != "jog-controlled-replacement-registry-v1" or len(registry["jobs"]) != 72:
        raise ValueError("Invalid 72-job controlled registry")
    ...
    try:
        ...
        print(json.dumps({"registered": 72, "already_verified": 72-len(pending),
        ...
        summary = {"schema": "jog-controlled-replacement-launcher-v1",
                   "status": "complete" if complete == 72 else "incomplete",
                   "created_utc": datetime.now(timezone.utc).isoformat(),
                   "registered": 72, "verified_complete": complete,
