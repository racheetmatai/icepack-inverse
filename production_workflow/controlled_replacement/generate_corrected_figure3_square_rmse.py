...
def select(path):
    ...
    return df.loc[df.experiment.str.fullmatch(r'SQ\d{2}') &
                  df.scenario.eq('controlled_original_measures')].sort_values(['experiment','configuration'])
df = select(args.metrics)
assert len(df) == 60 and not df.duplicated(['experiment','configuration']).any()
fields = ['ml_velocity_rmse_m_per_a','uniform_velocity_rmse_m_per_a','inversion_velocity_rmse_m_per_a']
...
record={'inputs':{str(args.metrics):hashlib.sha256(args.metrics.read_bytes()).hexdigest()},
        'verification':'All 60 corrected controlled-replacement results and both baselines are present.',
