...
from src.data_preprocessing import clean_imported_data, get_windowed_velocity_file, create_vertex_only_mesh_for_sparse_data, interpolate_data_onto_vertex_only_mesh, nearest_valid_velocity_samples, raster_window_dataarray, read_raster_file, stable_xy_row_ids
...
"""
    def __init__(self,
        ...
        self._bedmachine_dataset = xarray.open_dataset(thickness_filename)
        self.thickness = self._bedmachine_dataset["thickness"]
        self.surface = self._bedmachine_dataset["surface"]
        self.bed = self._bedmachine_dataset["bed"]
        self.bedmachine_source_data = self._bedmachine_dataset["source"]
        self.bedmachine_errbed_data = self._bedmachine_dataset["errbed"]
        self.degree = degree
    ...
    def _interpolate_raster_spec(
        self, specification, default_method="linear", force_method=False
    ):
        """Load, interpolate, and deterministically close a raster input."""
        method = default_method
        if isinstance(specification, Mapping) and not force_method:
            method = specification.get("method", default_method)
        if method not in {"linear", "nearest"}:
            raise ValueError(
                f"Unsupported raster interpolation method {method!r}; "
                "use 'linear' or 'nearest'."
            )
        raster = read_raster_file(specification)
        try:
            return icepack.interpolate(raster, self.Q, method=method)
        finally:
    ...
    def import_geophysics_data(self, name_list):
        ...
        Args:
            name_list (list): Raster paths or path/variable descriptors in
                order [magnetic anomaly, optional Bouguer anomaly, geothermal heat
                flux, surface air temperature, gravity disturbance, optional
                snow accumulation, englacial temperature, bed class]. Use
                ``None`` for Bouguer anomaly and/or snow accumulation when
                intentionally omitted. Continuous rasters default to linear interpolation.
                Bed class is always category-preserving nearest-neighbor
                interpolation.
        """
        if len(name_list) != 8:
            raise ValueError(
                "import_geophysics_data requires eight raster specifications; "
                f"received {len(name_list)}."
            )
        self.mag_anomaly = self._interpolate_raster_spec(name_list[0])
        self.boug_anomaly = (
            None
            if name_list[1] is None
            else self._interpolate_raster_spec(name_list[1])
        )
        self.heatflux = self._interpolate_raster_spec(name_list[2])

        surface_air_temp_celsius = read_raster_file(name_list[3])
        try:
            if isinstance(surface_air_temp_celsius, xarray.DataArray):
                surface_air_temp_da = surface_air_temp_celsius
            else:
                # Convert the raster data to xarray.DataArray
                surface_air_temp_da = convert_to_xarray(surface_air_temp_celsius)
        finally:
            close = getattr(surface_air_temp_celsius, "close", None)
            if close is not None:
                close()
        ...
        self.surface_air_temp = icepack.interpolate(surface_air_temp_da, self.Q)

        # Interpolate the temperature in Kelvin
        self.surface_air_temp_kelvin = icepack.interpolate(surface_air_temp_da_kelvin, self.Q)
        self.gravity_disturbance = self._interpolate_raster_spec(name_list[4])
        self.snow_accumulation = (
            None
            if name_list[5] is None
            else self._interpolate_raster_spec(name_list[5])
        )

        self.englacial_temp = self._interpolate_raster_spec(name_list[6])

        self.bed_class = (
            None
            if name_list[7] is None
            else self._interpolate_raster_spec(
                name_list[7], default_method="nearest", force_method=True
            )
        )
    ...
    def __del__(self):
        for name in ("vx_file", "vy_file", "stdx_file", "stdy_file"):
            close = getattr(getattr(self, name, None), "close", None)
            if close is not None:
                close()
        close = getattr(
            getattr(self, "_bedmachine_dataset", None), "close", None
        )
        if close is not None:
            close()
    ...
    def plot_accumulation(self, vmin=None, vmax=None):
        ...
        if self.snow_accumulation is None:
            raise ValueError(
                "Snow accumulation was intentionally omitted from this configuration."
            )
        fig, axes = self.plot_bounded_antarctica()
        axes.set_xlabel("meters")
        colors = firedrake.tripcolor(
    ...
    def compute_driving_stress(self):
        expr = driving_stress_mpa(
            self.h, firedrake.sqrt(inner(grad(self.s), grad(self.s)))
        )
    ...
    def compute_C_driving_stress(self):
        ...
        expr = driving_stress_mpa(
            self.h, firedrake.sqrt(inner(grad(self.s), grad(self.s)))
        ) / (firedrake.sqrt(inner(self.u_initial, self.u_initial)) ** (1/self.m))
    ...
    def compute_features(self, u=None, max_tu1_threshold=1998.30, max_tu2_threshold=555.53, max_tu3_threshold=593.56, max_tu4_threshold=859.16, max_tu5_threshold=1088.52, tolerance_bed=0.3):
        ...
        grad_h_fcn = firedrake.interpolate(grad_h, self.V)
        grad_s_fcn = firedrake.interpolate(grad_s, self.V)
        grad_b_fcn = firedrake.interpolate(grad_b, self.V)
        ...
        grad_h_npy = np.asarray(grad_h_fcn.dat.data_ro)
        grad_s_npy = np.asarray(grad_s_fcn.dat.data_ro)
        grad_b_npy = np.asarray(grad_b_fcn.dat.data_ro)
        magh_fcn_npy = np.linalg.norm(grad_h_npy, axis=1)
        mags_fcn_npy = np.linalg.norm(grad_s_npy, axis=1)
        magb_fcn_npy = np.linalg.norm(grad_b_npy, axis=1)
        cos_theta_bs_npy = stabilized_gradient_alignment(
            grad_b_npy[:, 0],
            grad_b_npy[:, 1],
            grad_s_npy[:, 0],
            grad_s_npy[:, 1],
        ...
        boug_anomaly_npy = (
            None if self.boug_anomaly is None else self.boug_anomaly.dat.data[:]
        )
        ...
        snow_accumulation_npy = (
            None
            if self.snow_accumulation is None
            else self.snow_accumulation.dat.data[:]
        )
        ...
        bedmachine_source_npy, bedmachine_errbed_npy = (
            self.sample_bedmachine_auxiliaries(x_npy, y_npy)
        )
        ...
        cluster_columns = {
            'x': x_npy,
            'y': y_npy,
            'invariant1': inv1_fcn_npy,
            'invariant2': inv2_fcn_npy,
            'mag_h': magh_fcn_npy,
            'mag_s': mags_fcn_npy,
            'mag_b': magb_fcn_npy,
            'cos_theta_bs': cos_theta_bs_npy,
            'h': h_npy,
            's': s_npy,
            'b': b_npy,
            'vel_mag': vel_mag_fcn_npy,
            'C': C_npy,
            'theta': theta_npy,
            ...
            'bedmachine_source': bedmachine_source_npy,
            'bedmachine_errbed': bedmachine_errbed_npy,
        }
        if self.bed_class is not None:
            cluster_columns['bed_class'] = self.bed_class.dat.data[:]
        if boug_anomaly_npy is not None:
            cluster_columns['boug_anomaly'] = boug_anomaly_npy
        if snow_accumulation_npy is not None:
            cluster_columns['snow_accumulation'] = snow_accumulation_npy
        cluster_df_full = pandas.DataFrame(cluster_columns)
        # Icepack-native stress unit: MPa.
        cluster_df_full['driving_stress'] = driving_stress_mpa(
            cluster_df_full['h'], cluster_df_full['mag_s']
        )
    ...
    def import_velocity_data(self, name = None, modified_exists = True, C = 'constant', constant_val = 1e-3):
        ...
        (
            ...
            self.velocity_valid_mask,
            self.velocity_error_valid_mask,
            self.velocity_mask_summary,
            self.window,
            self.transform,
        ) = get_windowed_velocity_file(
            name,
            self.outline,
            self.δ,
        ...
        self.Δ, self.indices = create_vertex_only_mesh_for_sparse_data(
            self.mesh,
            self.window,
            self.transform,
            valid_velocity=self.velocity_valid_mask,
        ...
        selected_error_valid = self.velocity_error_valid_mask[
            self.indices[:, 1], self.indices[:, 0]
        ]
        self.velocity_mask_summary["selected_observations"] = int(self.N)
        self.velocity_mask_summary["selected_with_valid_error_pair"] = int(
            np.count_nonzero(selected_error_valid)
        )
        velocity_x_centered = raster_window_dataarray(
            self.vx, self.transform, name="VX"
        )
        velocity_y_centered = raster_window_dataarray(
            self.vy, self.transform, name="VY"
        )
        error_x_centered = raster_window_dataarray(
            self.stdx, self.transform, name="ERRX"
        )
        error_y_centered = raster_window_dataarray(
            self.stdy, self.transform, name="ERRY"
        )
        self.velocity_full_grid_coordinate_mode = "cell_center"
        self.u_initial = icepack.interpolate(
            (velocity_x_centered, velocity_y_centered), self.V
        )
        initial_values = self.u_initial.dat.data
        initial_invalid = ~np.isfinite(initial_values).all(axis=1)
        coordinate_field = firedrake.interpolate(self.mesh.coordinates, self.V)
        initial_coordinates = np.asarray(
            coordinate_field.dat.data_ro[:, :2], dtype="float64"
        )
        boundary_nodes = firedrake.DirichletBC(
            self.V, firedrake.Constant((0.0, 0.0)), self.drichlet_ids
        ).nodes
        boundary_invalid = initial_invalid[boundary_nodes]
        fill_distances = np.asarray([], dtype="float64")
        if initial_invalid.any():
            fill_values, fill_distances = nearest_valid_velocity_samples(
                initial_coordinates[initial_invalid],
                self.vx,
                self.vy,
                self.velocity_valid_mask,
                self.transform,
            )
            initial_values[initial_invalid, :] = fill_values
        self.velocity_initial_fill_summary = {
            "policy": "nearest_valid_centered_pixel_for_solver_initial_field_only",
            "filled_dofs": int(initial_invalid.sum()),
            "dirichlet_filled_dofs": int(boundary_invalid.sum()),
            "maximum_distance_m": (
                float(fill_distances.max()) if len(fill_distances) else 0.0
            ),
            "median_distance_m": (
                float(np.median(fill_distances)) if len(fill_distances) else 0.0
            ),
            "observations_affected": 0,
        }
        self.u_exp_x = icepack.interpolate(velocity_x_centered, self.Q)
        self.u_exp_y = icepack.interpolate(velocity_y_centered, self.Q)
        ...
        if C == 'constant':
            ...
            print('C0 is constant:',constant_val)
            self.compute_C(constant_val = constant_val)
        self.σx = icepack.interpolate(error_x_centered, self.Q)
        self.σy = icepack.interpolate(error_y_centered, self.Q)
    ...
    def loss_functional(self, u):
        ...
        if not np.all(selected_error_valid):
            invalid_count = int(np.count_nonzero(~selected_error_valid))
            raise ValueError(
                "Uncertainty-weighted velocity loss requires positive finite "
                f"ERRX and ERRY at every observation; {invalid_count} selected "
                "observations do not satisfy that requirement. Use the unweighted "
                "nosigma loss or an explicitly error-valid observation mask."
            )
    ...
    def invert_C(
        self,
        gradient_tolerance=1e-100,
        step_tolerance=1e-100,
        max_iterations=50,
        loss_fcn_type='regular',
        regularization_grad_fcn=False,
        ...
        **kwargs,
        ...
        if initial_control is None:
            ...
            if initial_control.function_space() != self.Q:
                raise ValueError(
                    "Initial C control must use the inversion scalar function space."
                )
            self.C = initial_control.copy(deepcopy=True)
        problem = StatisticsProblem(
        ...
        self.last_C_estimator = estimator
        if return_estimator:
            return estimator
    ...
    def sample_bedmachine_auxiliaries(self, x, y):
        """Sample native BedMachine provenance fields at projected points.

        ``source`` is categorical and therefore sampled nearest-neighbor.
        ``errbed`` is continuous and sampled linearly. These fields are
        diagnostics only and never define observation or predictor eligibility.
        """
        x_indexer = xarray.DataArray(np.asarray(x, dtype="float64"), dims="point")
        y_indexer = xarray.DataArray(np.asarray(y, dtype="float64"), dims="point")
        source = self.bedmachine_source_data.sel(
            x=x_indexer, y=y_indexer, method="nearest"
        ).to_numpy()
        errbed = self.bedmachine_errbed_data.interp(
            x=x_indexer, y=y_indexer, method="linear"
        ).to_numpy()
        return np.asarray(source), np.asarray(errbed, dtype="float64")
    ...
    def get_dataframe(self, u):
        ...
        bed_class_1 = (
            None
            if self.bed_class is None
            else icepack.interpolate(self.bed_class, self.Δ)
        )
        ...
        C_npy = C_1.dat.data[:]
        bed_class_npy = (
            None if bed_class_1 is None else bed_class_1.dat.data[:]
        )
        ...
        boug_anomaly_npy = None
        if self.boug_anomaly is not None:
            boug_anomaly_npy = icepack.interpolate(
                self.boug_anomaly, self.Δ
            ).dat.data[:]
        ...
        snow_accumulation_npy = None
        if self.snow_accumulation is not None:
            snow_accumulation_npy = icepack.interpolate(
                self.snow_accumulation, self.Δ
            ).dat.data[:]
        ...
        velocity_source_npy = self.velocity_source[
            self.indices[:, 1], self.indices[:, 0]
        ]
        velocity_error_valid_npy = self.velocity_error_valid_mask[
            self.indices[:, 1], self.indices[:, 0]
        ]
        bedmachine_source_npy, bedmachine_errbed_npy = (
            self.sample_bedmachine_auxiliaries(x, y)
        )
        ...
        dataframe_columns = {
            ...
            'err_x': err_x_npy,
            'err_y': err_y_npy,
            'velocity_source': velocity_source_npy,
            'velocity_error_valid': velocity_error_valid_npy,
            'h': h_npy,
            's': s_npy,
            'b': b_npy,
            'mag_anomaly': mag_anomaly_npy,
        ...
        if boug_anomaly_npy is not None:
            dataframe_columns['boug_anomaly'] = boug_anomaly_npy
        if bed_class_npy is not None:
            dataframe_columns['bed_class'] = bed_class_npy
        if snow_accumulation_npy is not None:
            dataframe_columns['snow_accumulation'] = snow_accumulation_npy
        df = pandas.DataFrame(dataframe_columns)
        df.insert(0, 'row_id', stable_xy_row_ids(df['x'], df['y']))
        if df['row_id'].duplicated().any():
            raise ValueError(
                "Duplicate projected x/y coordinates produced duplicate row_id values."
            )
        ...
        df['cos_theta_bs'] = stabilized_gradient_alignment(
            df['grad_b_1'],
            df['grad_b_2'],
            df['grad_s_1'],
            df['grad_s_2'],
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            weighted_error = np.sqrt(
                ((df['x_velocity'] - df['x_velocity_initial']) / df['err_x'])**2
                + ((df['y_velocity'] - df['y_velocity_initial']) / df['err_y'])**2
            )
        df['total_u_error'] = np.where(
            df['velocity_error_valid'], weighted_error, np.nan
        )
        df['vel_mag'] = np.sqrt(df['x_velocity']**2 + df['y_velocity']**2)
        # Keep sparse training rows numerically identical to full-mesh inference.
        # Icepack's native stress unit is MPa.
